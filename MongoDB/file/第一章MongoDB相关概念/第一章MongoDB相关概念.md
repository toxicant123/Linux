...
|   32位整数   |       类型不可用。JavaScript Shell仅支持64位浮点数，所以32位整数会被自动转换        | JavaScript Shell是不支持该类型的，JavaScript Shell中默认会转换成64位浮点数 |
|   64位整数   |        不支持这个类型。JavaScript Shell会使用一个特殊的内嵌文档来显示64位整数        | JavaScript Shell是不支持该类型的，JavaScript Shell中默认会转换成64位浮点数 |
|  64位浮点数   |                JavaScript Shell中的数字就是这一种类型                 |                  {"x"：3.14159，"y"：3}                   |
|   null    |                        表示空值或者未定义的对象                        |                       {"x":null}                       |
| undefined |                       文档中也可以使用未定义类型                        |                    {"x":undefined}                     |
|    符号     | JavaScript Shell不支持，JavaScript Shell会将数据库中的符号类型的数据自动转换成字符串 |                                                        |
|   正则表达式   |             文档中可以包含正则表达式，采用JavaScript的正则表达式语法              |                   {"x" ： /foobar/i}                    |
|    代码     |                    文档中还可以包含JavaScript代码                    |            {"x" ： function() { /* …… */ }}             |
|   二进制数据   |          二进制数据可以由任意字节的串组成，不过JavaScript Shell中无法使用          |                                                        |
|  最大值/最小值  |       BSON包括一个特殊类型，表示可能的最大值。JavaScript Shell中没有这个类型        |                                                        |

提示：
JavaScript Shell默认使用64位浮点型数值。{“x”：3.14}或{“x”：3}。对于整型值，可以使用NumberInt（4字节符号整数）或NumberLong（8字节符号整数），{“x”:NumberInt(“3”)}{“x”:NumberLong(“3”)}
